...
**Source:** 半格 / HalfBit (@justhalfbit)  
**Published:** May 28, 2026  
**Original link:** https://x.com/justhalfbit/status/2059988985255215288
...
**仓库地址：** https://github.com/justhalfbit/ghostty-terminal-config
...
你开始输入命令的时候，它会在光标后面用灰色显示一条历史命令建议。如果就是你想要的，按 **→ 键**或者 **Ctrl+F** 一键接受整条命令。
...
> 💡 **隐藏用法：** 在命令后面输入两个星号再按 Tab 触发 fzf 补全。比如输入 `cd **` 然后按 Tab，会弹出 fzf 搜索所有子目录；输入 `kill **` 按 Tab，会列出所有进程让你选。
...
```bash
z ghost
# 直接跳到 ~/UserData/GitHubProjects/ghostty-terminal-config

z obsi
# 直接跳到 ~/UserData/ObsidianProjects
...
README 里有完整的手动安装步骤、配置文件说明、快捷键速查表。

> ⭐ 觉得有用请 star，问题和建议欢迎提 issue。  
> 📋 关注 @justhalfbit，持续更新踩坑指南  
> 🧩 懂半格，刚好够分享  
> 💬 有问题随时问，看到都回  
> 🔄 觉得有用？转给同样需要的朋友
...
*Compiled from X user 半格 / HalfBit (@justhalfbit)*  
*原文发布于 2026-05-28*
