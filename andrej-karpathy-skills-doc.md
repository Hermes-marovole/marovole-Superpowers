# Andrej Karpathy 编码指南：andrej-karpathy-skills 项目详解

> 来源：X @jiayuan_jy (Jiayuan Zhang) — 2026-05-22
> 项目地址：https://github.com/multica-ai/andrej-karpathy-skills
> 当前状态：已进入 GitHub 历史 Star 数前 50 项目

---

## 一、项目背景

2026 年 5 月 22 日，Jiayuan (JY) Zhang 在 X 上宣布，其维护的开源项目 `andrej-karpathy-skills` 已成为 **GitHub 历史 Star 数排名前 50** 的项目。该项目最初源于 Andrej Karpathy（前 Tesla AI 总监、OpenAI 创始成员）对 LLM 编码陷阱的深度观察，后由 Jiayuan Zhang 整理为一份可直接用于 Claude Code 的 `CLAUDE.md` 指南文件。

### Andrej Karpathy 指出的核心问题
...
该项目将解决方案浓缩为**四个原则**，集中在一个文件中直接解决上述问题：

- **编码前思考** — 解决错误假设、隐藏困惑、缺少权衡
- **简洁优先** — 解决过度复杂、臃肿抽象
- **精准修改** — 解决无关编辑、触碰不应碰的代码
- **目标驱动执行** — 通过测试优先、可验证的成功标准来驱动
...
- **Multica** —— Jiayuan Zhang 的新项目，一个用于运行和管理编码智能体的开源平台，支持可复用的技能：https://github.com/multica-ai/multica
- **原始推文** —— https://x.com/jiayuan_jy/status/2057771113837703594
